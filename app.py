        ...
        try:
            ...
            if "organic" in result:
                for item in result["organic"]:
                    ...
                    if emails:
                        return emails[0], "🟢 已找到", link
                        
            return "未找到", "🔴 需人工核查", "无来源"
            
        except Exception as e:
            return "错误", f"⚠️ 接口异常: {str(e)}", "无来源"

    # ==========================================
    # 🇨🇳 中文逻辑 (100% 还原你提供的打分版代码)
    # ==========================================
    else:
        ...
        try:
            ...
            if "organic" in result:
                ...
                for email, data in email_candidates.items():
                    ...
                    if "163.com" in domain or "qq.com" in domain:
                        score += 10
...
# 云端读写函数
# ==========================================
@st.cache_data(ttl=60) # 加上缓存，防止读取过于频繁
...
def save_to_custom_db(url, payload):
    try:
        response = requests.post(url, json=payload, timeout=10)
        if response.status_code == 200:
            st.toast(f"✅ 云端同步成功！", icon="☁️")
        else:
            st.error(f"❌ 同步失败！服务器返回状态码: {response.status_code}")
    except Exception as e:
        st.error(f"⚠️ 无法连接到云端数据库: {str(e)}")
...
with tab1:
    ...
    # ==========================================
            ...
            if not st.session_state.api_key or not st.session_state.db_url:
                ...
                with st.spinner(f"正在全网扫描 {single_name} 的联系方式..."):
                    ...
                    st.session_state.single_result = {
                        "email": email, "status": status, "source_url": source_url,
                        "name": single_name, "inst": single_inst
                    }
        ...
        # 只要内存里有结果，就直接渲染出来（防止刷新丢失）
        if st.session_state.single_result:
            res = st.session_state.single_result
            st.markdown("---")
            if res["email"] != "未找到":
                st.success(f"🎉 **成功获取联系方式！** ({res['status']})")
                st.info(f"📧 **提取邮箱**: `{res['email']}`\n\n🔗 **溯源核查**: [点击访问网页来源]({res['source_url']})")
                
                st.markdown("### ✍️ 专属邮件撰写台")
                
                clean_n = res["name"].replace(',', ' ')
                last_name_guess = res["name"].split(',')[0].strip() if ',' in res["name"] else clean_n.split()[-1]
                
                default_subject = f"Inquiry regarding your research at {res['inst']}"
                default_body = (
                    f"Dear Prof. {last_name_guess},\n\n"
                    f"I hope this email finds you well. I have been following your excellent work at {res['inst']}.\n\n"
                    f"[此处填写您对教授研究的具体见解或提问]\n\n"
                    f"Best regards,\n"
                    f"{st.session_state.real_name}\n"
                    f"{st.session_state.dept_name}"
                )
                ...
                mailto_link = f"mailto:{res['email']}?subject={safe_subject}&body={safe_body}"
                ...
                fallback_url = f"https://www.google.com/search?q={urllib.parse.quote(res['name'] + ' ' + res['inst'] + ' email')}"
            ...
            if success_df.empty:
                ...
                with col_tmpl2:
                    ...
                    with st.popover("➕ 导入/新增自定义模板", use_container_width=True):
                        ...
                        new_content = st.text_area("2. 模板内容", placeholder="可用 {name} 代表学者姓名，用 {signature} 代表您的落款", height=200)
                    ...
                    with st.expander(f"👤 发送给：{name} ({email})"):
                        ...
                        st.markdown(f'''
                            <a href="{link}" target="_blank">
                                <button style="background-color: #4CAF50; color: white; padding: 10px 24px; border: none; border-radius: 8px; font-size: 16px; font-weight: bold; cursor: pointer; width: 100%; transition: 0.3s;">📧 唤起本地邮箱发送</button>
                            </a>
                        ''', unsafe_allow_html=True)
        else:
...
with tab3:
    if not st.session_state.db_url:
        ...
        if isinstance(db_result, str):
            ...
            db_result.columns = [str(c).strip() for c in db_result.columns]
